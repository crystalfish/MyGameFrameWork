...
namespace COMMON
{
    ...
    {
        public class XBinaryWriter : System.IO.BinaryWriter
        {
            ...
            //unsafe public override void Write(float value)
            //{
            //    base.Write(*(int*)(&value));
            //}

            //unsafe public override void Write(double value)
            //{
        ...
        public class XBinaryReader : System.IO.BinaryReader
        {
            ...
            {
                tmp.i = base.ReadInt32();
                return tmp.f;
            }

            public override double ReadDouble()
            {
                tmp.l = base.ReadInt64();
                return tmp.d;
            }

            //unsafe public override float ReadSingle()
            //{
            //    var i = base.ReadInt32();
            //    return *(float*)(&i);
            //}

            //unsafe public override double ReadDouble()
            //{
        ...
        public void ReadVector3(ref UnityEngine.Vector3 v)
        {
            v.x = In.ReadSingle();
            v.y = In.ReadSingle();
            v.z = In.ReadSingle();
        ...
        public void ReadColor(ref UnityEngine.Color c)
        {
            c.r = In.ReadSingle();
            c.g = In.ReadSingle();
            c.b = In.ReadSingle();
        ...
        public void WriteColor(ref UnityEngine.Color c)
        {
            Out.Write(c.r);
            Out.Write(c.g);
            Out.Write(c.b);
        ...
        public void WriteVector3(ref UnityEngine.Vector3 v)
        {
            Out.Write(v.x);
            Out.Write(v.y);
            Out.Write(v.z);
        ...
        /// <returns></returns>
        public List<string> ReadStrings(List<string> output = null)
        {
            var reader = In;
            var N = reader.ReadInt32();
            if(null == output)
            {
                output = new List<string>(N);
            }
            for(var i = 0; i < N;++i)
            {
        ...
        public static byte[] OpenRead(string filePath,byte[] cached = null)
        {
            ...
            {
                Logger.Log("File not found:{0}",filePath);
            }
            ...
            else
            {
                if(cached.Length < N)
                {
                    Array.Resize<byte>(ref cached, N);
                }
            }
    ...
    public static class BinaryWritterExt2
    {
        public static void WriteAsciiString(this BinaryWriter bw, string str)
        {
            bw.Write(str.Length + 1);
            var tmp = System.Text.Encoding.ASCII.GetBytes(str);
            var tmp2 = new byte[tmp.Length + 1];
            Buffer.BlockCopy(tmp, 0, tmp2, 0, tmp.Length);
